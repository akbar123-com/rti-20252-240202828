...
| Method | Data detak jantung di-generate menggunakan mikrokontroler dan sensor MAX30102 yang dikirim langsung ke web server lokal. Pengumpulan data dilakukan sebanyak 35 run berpasangan (paired) untuk tiap database pada kondisi yang identik, kemudian dianalisis perbedaan rata-ratanya menggunakan Paired Samples T-Test (untuk Latency) dan Wilcoxon Signed-Rank Test (untuk RAM, karena selisihnya tidak berdistribusi normal). | 800-1200 |
| Results | Paired Samples T-Test membuktikan MySQL secara statistik signifikan lebih cepat dengan rata-rata waktu simpan 2.12 ms dibanding PostgreSQL yang mencapai 4.46 ms (p < 0.001, Cohen's d = -4.01). Sebaliknya, uji Wilcoxon pada RAM (86.65% vs 86.25%) tidak menunjukkan perbedaan signifikan (p = 0.368). Pada metrik data loss, MySQL mencatat total 35 baris hilang (rata-rata 1.0/run, 23 dari 35 run tanpa loss, dengan satu lonjakan ekstrem 15 baris pada satu run), sedangkan PostgreSQL mencatat total 41 baris hilang (rata-rata 1.17/run, hanya 15 dari 35 run tanpa loss). | 500-800 |
| Discussion | Latency menunjukkan trade-off yang jelas dan terbukti signifikan: MySQL jauh lebih cepat, sedangkan RAM ternyata tidak berbeda signifikan antar kedua sistem — sehingga tidak ada trade-off RAM yang bisa diklaim. Pada data loss, pola justru berlawanan dari asumsi umum: PostgreSQL kehilangan data lebih *sering* (41 baris total, 20 dari 35 run mengalami loss) meski dalam jumlah kecil per insiden, sementara MySQL jarang kehilangan data (23 dari 35 run bersih) tapi rentan terhadap satu lonjakan ekstrem (bottleneck 15 baris). Ini berarti karakteristik kegagalan kedua sistem berbeda pola, bukan sekadar "MySQL cepat-tidak-stabil vs PostgreSQL lambat-stabil" — klaim reliabilitas perlu diuji signifikansinya secara statistik (mis. Mann-Whitney/Wilcoxon pada variabel loss) sebelum disimpulkan lebih lanjut.| 600-900 |
| Conclusion | MySQL direkomendasikan untuk arsitektur IoT medis yang memprioritaskan kecepatan real-time, karena keunggulan latensinya terbukti signifikan secara statistik. Namun, perancang sistem harus menyiapkan mekanisme penanganan antrean (queue handling) untuk mencegah lonjakan data loss ekstrem seperti yang teramati pada MySQL. Klaim bahwa PostgreSQL "lebih andal" tidak dapat disimpulkan dari data ini — justru PostgreSQL kehilangan data pada proporsi run yang lebih besar, sehingga pengujian statistik lanjutan terhadap variabel data loss diperlukan sebelum trade-off kecepatan-vs-keandalan dapat diklaim sebagai temuan definitif. | 200-400 |
...
> Pengujian ini mengevaluasi latensi dan keandalan MySQL dan PostgreSQL dalam menangani aliran data frekuensi tinggi dari sensor MAX30102. Uji statistik menunjukkan MySQL memiliki rata-rata waktu simpan yang secara signifikan lebih cepat (2.12 ms) dibandingkan PostgreSQL (4.46 ms). Pada aspek keandalan, MySQL mengalami satu lonjakan data loss ekstrem sebanyak 15 baris pada satu run, sementara PostgreSQL kehilangan data secara lebih sering meski dalam jumlah kecil per insiden (total 41 baris dari 35 run) — sehingga tidak ada satu sistem yang terbukti lebih andal secara keseluruhan tanpa pengujian statistik lebih lanjut.
---
