...
| 7 | Discussion: Latency terbukti signifikan lebih cepat di MySQL, namun RAM tidak berbeda signifikan — dan pola data loss kedua DB berbeda karakter (bukan sekadar "MySQL cepat-rentan vs PostgreSQL lambat-aman").|Infografis perbandingan pola kegagalan tiap DB. |1.5 min |
...
| 4 | Discussion| Jika PostgreSQL terbukti lebih lambat, apakah artinya arsitektur ini gagal dan tidak layak dipakai untuk IoT Medis?|Lambat tidak berarti gagal — namun klaim "PostgreSQL lebih stabil/aman" juga tidak terbukti dari data ini. |Data loss tercatat pada **kedua** database: MySQL total 35 baris (23 dari 35 run bersih, dengan satu lonjakan ekstrem 15 baris), PostgreSQL total **41 baris** (hanya 15 dari 35 run bersih, tanpa lonjakan ekstrem). Uji RAM (Wilcoxon) juga tidak signifikan (p=0,368). | Pola kegagalan kedua sistem berbeda karakter — MySQL jarang gagal tapi bisa ekstrem, PostgreSQL sering gagal kecil-kecilan — sehingga klaim "keandalan mutlak" tidak bisa diambil hanya dari observasi deskriptif; perlu uji statistik formal pada variabel data loss sebelum menyimpulkan arsitektur mana yang benar-benar lebih aman.|
| 5 |Conclusion |Berdasarkan kelebihan dan kekurangan ini, apa solusi paling aman jika sistem ini akan benar-benar dipasang di Rumah Sakit besok? |Dibutuhkan pendekatan hybrid menggunakan sistem antrean (queueing), bukan memilih satu database berdasarkan asumsi "yang satu pasti lebih aman". | MySQL unggul signifikan di latensi (2,12 ms vs 4,46 ms, p<0,001, Paired Samples T-Test) namun rentan lonjakan loss ekstrem; PostgreSQL tidak unggul di RAM (tidak signifikan) dan justru lebih sering (walau lebih kecil per insiden) mengalami data loss.| Karena tidak ada satu sistem yang terbukti unggul di semua aspek keandalan, menambahkan message broker (MQTT/Redis) sebagai buffer di depan MySQL memberi kombinasi kecepatan tinggi + perlindungan dari loss, tanpa bergantung pada asumsi "PostgreSQL lebih aman" yang tidak terbukti secara statistik.|
